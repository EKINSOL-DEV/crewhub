...
class GatewayClient:
    ...
    async def send_chat(self, message: str, agent_id: str = "main", session_id=None, timeout: float = 120.0):  # NOSONAR
        conn = await self._get_conn()
    ...
    async def call(self, method, params=None, timeout=30.0, **kwargs):  # NOSONAR
        conn = await self._get_conn()
