...
Big update dropping today — **v0.14.0** brings a cleaner Zen Mode architecture, hands-on prop editing, and developer-friendly debugging tools!
...
### Dev Error Logger 🐛
A built-in debugging overlay for development:
- **Runtime error capture** — catches and displays errors with full stack traces
- **Error overlay** — non-intrusive viewer that shows errors in context
- **DevErrorViewer component** — toggle on/off, browse error history
- **Better DX** — no more hunting through console for cryptic errors

## 📦 Also Included

- **PropMaker Quality Phase 2-3** — Component library (7 reusable animated parts: LED, Steam, GlowOrb, Cable, DataStream, Screen, RotatingPart), multi-pass generation pipeline, iteration system, style transfer, hybrid generation, quality scorer
