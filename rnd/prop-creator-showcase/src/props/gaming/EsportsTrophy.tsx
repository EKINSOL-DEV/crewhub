...
export function EsportsTrophy() {
  ...
  useFrame((s) => {
    ...
    if (starRef.current) { const sc = 1 + Math.sin(s.clock.elapsedTime * 2) * 0.1; starRef.current.scale.set(sc, sc, sc); }
  });
  return (
    <group ref={groupRef}>
      ...
      {[-0.08, 0.08].map((x,i) => (
        <mesh key={i} position={[x, -0.06, 0]}><torusGeometry args={[0.03, 0.008, 4, 6, Math.PI]} /><meshStandardMaterial color="#cc9933" flatShading /></mesh>
      ))}
