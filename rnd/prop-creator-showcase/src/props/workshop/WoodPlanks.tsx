...
export function WoodPlanks() {
  ...
  return (
    <group ref={groupRef}>
      {/* Planks stacked */}
      {[0, 1, 2, 3, 4].map(i => (
        <mesh key={i} position={[i * 0.02 - 0.04, -0.4 + i * 0.06, i * 0.01]}>
      ...
      {Array.from({ length: 10 }, (_, i) => (
        <mesh key={i} position={[(Math.random() - 0.5) * 0.3, -0.45, (Math.random() - 0.5) * 0.2 + 0.15]}>
      ...
      {/* Wood grain lines */}
      {[0, 2, 4].map(i => (
        <mesh key={i} position={[0, -0.37 + i * 0.06, 0.076]}>
