  ...
  return (
    <group ref={groupRef}>
      ...
      {Array.from({length: 8}).map((_,i) => (
        <mesh key={JSON.stringify(_)} position={[-0.12+(i%4)*0.08, -0.18, -0.04+Math.floor(i/4)*0.08]}>
          <cylinderGeometry args={[0.015+i*0.002, 0.015+i*0.002, 0.02, 6]} />
          <meshStandardMaterial color="#aaaacc" flatShading />
        </mesh>
      ))}
      <mesh position={[0.12, -0.18, 0]}><boxGeometry args={[0.04, 0.015, 0.12]} /><meshStandardMaterial color="#aaaacc" flatShading /></mesh>
