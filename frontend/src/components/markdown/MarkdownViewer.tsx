...
export function MarkdownViewer({ content, className, maxHeight }: MarkdownViewerProps) {
  const components = useMemo(() => ({
    ...
    p: ({ children }: any) => (
      <p style={{ marginBottom: 12, lineHeight: 1.7, color: 'var(--zen-fg, hsl(var(--foreground)))' }}>{children}</p>
    ),
    ul: ({ children }: any) => (
    ...
    li: ({ children }: any) => (
      <li style={{ marginBottom: 4, lineHeight: 1.6, color: 'var(--zen-fg, hsl(var(--foreground)))' }}>{children}</li>
    ),
    a: ({ href, children }: any) => (
      <a href={href} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--zen-accent, hsl(var(--primary)))', textDecoration: 'underline' }}>{children}</a>
    ),
    blockquote: ({ children }: any) => (
      <blockquote style={{ borderLeft: '3px solid var(--zen-accent, hsl(var(--primary)))', paddingLeft: 16, margin: '12px 0', color: 'var(--zen-fg-muted, hsl(var(--muted-foreground)))', fontStyle: 'italic' }}>{children}</blockquote>
    ),
    table: ({ children }: any) => (
    ...
    th: ({ children }: any) => (
      <th style={{ border: '1px solid var(--zen-border, hsl(var(--border)))', padding: '8px 12px', textAlign: 'left', fontWeight: 600, background: 'var(--zen-bg-elevated, hsl(var(--secondary)))' }}>{children}</th>
    ),
    td: ({ children }: any) => (
      <td style={{ border: '1px solid var(--zen-border, hsl(var(--border)))', padding: '8px 12px' }}>{children}</td>
    ),
    code: ({ className, children, ...props }: any) => {
      ...
      return (
        <code style={{ background: 'var(--zen-bg-elevated, hsl(var(--secondary)))', padding: '2px 6px', borderRadius: 4, fontSize: '0.9em', fontFamily: "'JetBrains Mono', 'Fira Code', monospace" }} {...props}>{children}</code>
      )
    },
    pre: ({ children }: any) => <>{children}</>,
    hr: () => <hr style={{ border: 'none', borderTop: '1px solid var(--zen-border, hsl(var(--border)))', margin: '24px 0' }} />,
    img: ({ src, alt }: any) => {
      // Only render remote images
      if (src?.startsWith('http')) {
        return <img src={src} alt={alt || ''} style={{ maxWidth: '100%', borderRadius: 8, margin: '12px 0' }} />
      }
      return <span style={{ color: 'var(--zen-fg-muted, hsl(var(--muted-foreground)))', fontStyle: 'italic' }}>[Image: {alt || src}]</span>
    },
