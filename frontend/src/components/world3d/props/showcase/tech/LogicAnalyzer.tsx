    ...
    if (ledsRef.current)
      ledsRef.current.children.forEach((led, i) => {
        ;((led as THREE.Mesh).material as any).emissiveIntensity =
          Math.sin(s.clock.elapsedTime * 5 + i * 0.7) > 0 ? 2 : 0.1
      })
  })
  return (
    <group ref={groupRef}>
      ...
      <group ref={ledsRef}>
        {Array.from({ length: 8 }).map((_, i) => (
          <mesh key={JSON.stringify(_)} position={[-0.1 + i * 0.03, -0.165, 0.06]}>
      ...
      {Array.from({ length: 8 }).map((_, i) => (
        <mesh key={JSON.stringify(_)} position={[-0.1 + i * 0.03, -0.2, 0.065]}>
