  ...
  useFrame((s) => {
    ...
    if (ledsRef.current)
      ledsRef.current.children.forEach((led, i) => {
        ;((led as THREE.Mesh).material as any).emissiveIntensity =
          Math.sin(s.clock.elapsedTime * 3 + i * 0.5) > 0 ? 2 : 0.2
      })
  })
  return (
    <group ref={groupRef}>
      ...
      {Array.from({ length: 8 }).map((_, i) => (
        <mesh key={JSON.stringify(_)} position={[-0.16 + i * 0.046, -0.22, 0.076]}>
      ...
      <group ref={ledsRef}>
        {Array.from({ length: 8 }).map((_, i) => (
          <mesh key={JSON.stringify(_)} position={[-0.16 + i * 0.046, -0.195, 0.076]}>
