  ...
  return (
    <group ref={groupRef}>
      ...
      {[0.45, 0.05, -0.35].map((y, i) => (
        <mesh key={i} position={[0, y, 0]}><boxGeometry args={[0.95, 0.04, 0.33]} /><meshStandardMaterial color="#9B7924" flatShading /></mesh>
      ))}
      {/* Books row 1 */}
      {bookColors.slice(0, 4).map((c, i) => (
        <mesh key={i} position={[-0.3 + i * 0.18, 0.28, 0]}>
      ...
      {bookColors.slice(4).map((c, i) => (
        <mesh key={i} position={[-0.3 + i * 0.2, -0.12, 0]}>
