...
export function ConsoleStand() {
  ...
  return (
    <group ref={groupRef}>
      ...
      {[-0.08, 0.08].map((x,i) => (
        <mesh key={i} position={[x, -0.18, 0]}><boxGeometry args={[0.03, 0.04, 0.12]} /><meshStandardMaterial color="#222233" flatShading /></mesh>
      ))}
      ...
      {[-0.1, 0, 0.1].map((z,i) => (
        <mesh key={i} position={[0, -0.24, z]}><boxGeometry args={[0.2, 0.002, 0.01]} /><meshStandardMaterial color="#333344" flatShading /></mesh>
      ))}
