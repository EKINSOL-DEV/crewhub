...
export function DevToolbar() {
  ...
  const [visible, setVisible] = useState(() => {
    try {
      return localStorage.getItem('dev-error-viewer-visible') === 'true'
    } catch {
      return false
    }
  })
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontWeight: 600, fontSize: '1rem', color: '#c0caf5' }}>Dev Error Log</span>
          ...
          filtered.map((err) => {
            ...
            return (
              ...
              >
                {/* Error summary row */}
                <div
                  onClick={() = role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); (e.currentTarget as HTMLElement).click() } }}> setExpandedId(expanded ? null : err.id)}
                ...
                >
                  {expanded ? (
                    <ChevronDown
                      size={14}
                      style={{ marginTop: 2, flexShrink: 0, color: '#565f89' }}
                    />
                  ) : (
                    <ChevronRight
                      size={14}
                  ...
                  >
                    ...
                    {err.stack && (
                      <details open style={{ marginBottom: 8 }}>
                        <summary style={{ color: '#bb9af7', cursor: 'pointer', marginBottom: 4 }}>
                          Stack Trace
                        </summary>
                        <pre
                          style={{
