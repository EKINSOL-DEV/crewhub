...
function PixelCell({ 
  ...
  const backgroundColor = useMemo(() => {
    switch (value) {
      ...
      case 2: return '#ffffff'  // highlight (eyes)
      case 3: return colors.dark
...
export function PixelAvatar({ agentName, status, stats }: PixelAvatarProps) {
  ...
  const statusColorClass = status === 'error' 
    ...
    <div className={`pixel-avatar ${statusClass}`}>
      ...
      <div className="pixel-avatar-tooltip">
        ...
        <div className="pixel-avatar-tooltip-stat">
          ...
          <span className={`pixel-avatar-tooltip-value ${statusColorClass}`}>
            {statusText}
          </span>
        </div>
