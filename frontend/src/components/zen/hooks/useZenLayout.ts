...
export interface UseZenLayoutReturn {
  ...
  setPanelAgent: (panelId: string, sessionKey: string, agentName: string, agentIcon?: string) => void
}
...
export function useZenLayout(): UseZenLayoutReturn {
  ...
  const splitPanelAction = useCallback((panelId: string, direction: 'row' | 'col', newType: PanelType = 'empty') => {
    setState(prev => {
      ...
      const newPanel = newPanels.find(p => !getAllPanels(prev.root).some(op => op.panelId === p.panelId))
      return {
        ...prev,
        root: newRoot,
  ...
  const closePanel = useCallback((panelId: string) => {
    setState(prev => {
      ...
      const newPanels = getAllPanels(newRoot)
      const newFocusedId = panelId === prev.focusedPanelId
        ? newPanels[0]?.panelId || ''
        : prev.focusedPanelId
