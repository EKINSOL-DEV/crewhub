...
export function StandupHistory({ maxDays = 3 }: StandupHistoryProps) {
  ...
  if (standups.length === 0) {
    return (
      <div style={{
        padding: '16px 14px', background: 'rgba(79,70,229,0.04)',
      ...
      {standups.map(s => (
        <div key={s.id}>
          ...
          >
            ...
            <span style={{ fontSize: 12, color: '#9ca3af', transform: expanded === s.id ? 'rotate(90deg)' : 'none', transition: 'transform 0.15s' }}>
              ▶
            </span>
          </button>
