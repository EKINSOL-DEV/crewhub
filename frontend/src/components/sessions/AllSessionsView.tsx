...
function getSessionStatusForTable(
  ...
  if (now - lastUpdate < SESSION_CONFIG.tableActiveThresholdMs)
    return { label: 'Active', variant: 'default' }

  // Check for supervising (has active subagents)
  if (allSessions && hasActiveSubagents(session, allSessions))
    return { label: 'Supervising', variant: 'default' }

  if (now - lastUpdate < SESSION_CONFIG.tableIdleThresholdMs)
    ...
    result.sort((a, b) => {
      ...
      switch (sortField) {
        case 'name':
          ...
          break
        case 'status':
          comparison = getSessionStatusForTable(a, sessions).label.localeCompare(
            getSessionStatusForTable(b, sessions).label
          )
          break
        case 'model':
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col min-w-0">
                      ...
                      <span className="text-xs text-muted-foreground truncate">{session.key}</span>
                    </div>
                    <div>
