...
export function LogViewer({ session, open, onOpenChange }: LogViewerProps) {
  ...
  const scrollToBottom = () => { if (scrollContainerRef.current) scrollContainerRef.current.scrollTop = scrollContainerRef.current.scrollHeight }
  ...
  const exportToJSON = () => {
    if (!session) return
    const data = { session: { key: session.key, displayName: getSessionDisplayName(session), model: session.model, exportedAt: new Date().toISOString() }, messages, stats: { totalMessages: messages.length, totalTokens: messages.reduce((sum, msg) => sum + (msg.usage?.totalTokens || 0), 0), totalCost: messages.reduce((sum, msg) => sum + (msg.usage?.cost?.total || 0), 0) } }
  ...
  const filteredMessages = messages.filter(msg => {
    ...
    if (searchQuery) {
      ...
      return msg.content.some(block => (block?.text?.toLowerCase().includes(query)) || (block?.thinking?.toLowerCase().includes(query)) || (block?.name?.toLowerCase().includes(query)))
    }
    return true
  })
    ...
    >
      ...
      >
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground flex-wrap">
                ...
                <span>·</span><span>Last activity: {timeAgo(session.updatedAt)}</span>
                {session.totalTokens && <><span>·</span><span>{session.totalTokens.toLocaleString()} tokens</span></>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-3 border-b bg-muted/30 shrink-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="relative flex-1 max-w-xs">
              ...
              {searchQuery && <Button size="sm" variant="ghost" onClick={() => setSearchQuery("")} className="absolute right-0 top-0 h-8 w-8 p-0"><X className="h-3.5 w-3.5" /></Button>}
            </div>
            <Button size="sm" variant="outline" onClick={exportToJSON} className="h-8"><Download className="h-3.5 w-3.5 mr-1.5" />JSON</Button>
            <Button size="sm" variant="ghost" onClick={() => fetchMessages()} className="h-8 w-8 p-0" disabled={loading}><RefreshCw className={cn("h-3.5 w-3.5", isRefreshing && "animate-spin")} /></Button>
          </div>
        ...
        <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-6 py-4">
          ...
          : filteredMessages.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-center"><FileText className="h-12 w-12 text-muted-foreground/50 mb-3" /><p className="text-sm text-muted-foreground">{searchQuery || roleFilter !== "all" ? "No messages match your filters" : "No messages yet"}</p></div>
          : <div className="space-y-4">
              {filteredMessages.map((msg, idx) => (
                <div key={idx} className={cn("p-3 rounded-lg", msg.role === "user" ? "bg-blue-50 dark:bg-blue-950" : msg.role === "system" ? "bg-gray-50 dark:bg-gray-950" : "bg-green-50 dark:bg-green-950")}>
                  <div className="flex items-center gap-2 mb-2 text-xs text-muted-foreground">
                    ...
                    {msg.usage && <span className="ml-auto">{msg.usage.totalTokens} tokens · ${msg.usage.cost?.total.toFixed(4)}</span>}
                  </div>
                  <div className="text-sm whitespace-pre-wrap">
                    {msg.content?.map((block, bidx) => (
                      <div key={bidx}>
                        {block.type === "text" && block.text && <span>{block.text}</span>}
                        {block.type === "thinking" && block.thinking && <div className="text-purple-600 dark:text-purple-400 italic">💭 {block.thinking}</div>}
                        {(block.type === "tool_use" || block.type === "toolCall") && <div className="text-amber-600 dark:text-amber-400">🔧 {block.name}</div>}
                        {block.type === "tool_result" && <div className="text-gray-500">✓ Result</div>}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex items-center justify-center py-2 text-xs text-muted-foreground">
                <div className="flex items-center gap-2"><div className={cn("w-2 h-2 rounded-full", isRefreshing ? "bg-green-500 animate-pulse" : "bg-green-500")} /><span>Live - auto-refreshing every 3s</span></div>
              </div>
            </div>}
        </div>
        ...
        <div className="px-6 py-3 border-t bg-muted/30 text-xs text-muted-foreground shrink-0">
          <div className="flex items-center gap-4 justify-between flex-wrap">
            <div className="flex items-center gap-4">
              ...
              {messages.length > 0 && <><span>·</span><span>{messages.reduce((sum, msg) => sum + (msg.usage?.totalTokens || 0), 0).toLocaleString()} tokens</span><span>·</span><span>${messages.reduce((sum, msg) => sum + (msg.usage?.cost?.total || 0), 0).toFixed(4)} total</span></>}
            </div>
